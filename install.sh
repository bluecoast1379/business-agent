...
# 校验 Node.js >= 22 后,把参数转发给 bin/init-workspace.cjs。
...
if ! command -v node >/dev/null 2>&1; then
  echo "错误:未找到 node。business-agent 需要 Node.js >= 22,请先安装。" >&2
  exit 1
fi
...
if [ "$NODE_MAJOR" -lt 22 ]; then
  echo "错误:Node.js 版本过低($(node -v 2>/dev/null || echo unknown)),business-agent 需要 >= 22。" >&2
  exit 1
fi
