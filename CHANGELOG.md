...
> 说明:下方版本号只描述**仓库内容的目标版本**,处于 Unreleased 状态——不代表任何远程 tag、GitHub Release 或 npm 包已存在。对外分发前请按 [docs/install.md](./docs/install.md) 的通道与验收清单执行。
...
- **质量与安全检查**:`check:syntax`、`check:sanitized`(词面 + 密钥形态,支持仓库外 `--extra-banned` denylist)、`check:manifest`、`check:scaffold`(mock 冒烟)、`test:smoke`(init 冒烟),由 `npm run check` 串联。
...
- scaffold 配置 fail-fast,无任何兜底密钥或默认内部地址;写操作工具一律人工确认。
