...
| 零依赖 scaffold | Node 22+ ESM,只用 `node:http` 与全局 `fetch`;自带 Anthropic/Mock 双 provider、会话存储、成本追踪、scope 绑定、写操作确认闸、定时巡检与 demo 工具包 |
...
任一失败整体非零退出。kit 与 scaffold 均无 npm 依赖,`node >= 22` 即可运行全部检查。
